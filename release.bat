...
set CURRENT_VER=
set CURRENT_CODE=
for /f "tokens=2" %%a in ('findstr /C:"versionName" android\app\build.gradle') do (
    if "!CURRENT_VER!"=="" set CURRENT_VER=%%~a
)
for /f "tokens=2" %%a in ('findstr /C:"versionCode" android\app\build.gradle') do (
    if "!CURRENT_CODE!"=="" set CURRENT_CODE=%%a
)

cls
echo ========================================
echo   FoodMap (Android) Release Tool
echo ========================================
echo.
echo Current version: %CURRENT_VER% (versionCode %CURRENT_CODE%)

if "%CURRENT_VER%"=="" (
    echo ERROR: Cannot read versionName from android\app\build.gradle
    pause ^& exit /b 1
)
...
:: --- NEW RELEASE ---
:NEW_RELEASE
set /p NEW_VER=Enter new version (e.g. 1.1):
if "%NEW_VER%"=="" (
...
echo.
echo Will release v%NEW_VER% ^(current: v%CURRENT_VER%^), versionCode %CURRENT_CODE% -^> %NEW_CODE%
...
echo.
echo [1/4] Updating android\app\build.gradle...

:: Use [System.IO.File] to avoid UTF-8 BOM corruption
powershell -NoProfile -Command "$enc = New-Object System.Text.UTF8Encoding($false); $f = 'android\app\build.gradle'; [System.IO.File]::WriteAllText($f, ([System.IO.File]::ReadAllText($f) -replace 'versionName \"%CURRENT_VER%\"', 'versionName \"%NEW_VER%\"' -replace 'versionCode %CURRENT_CODE%', 'versionCode %NEW_CODE%'), $enc)"
if errorlevel 1 ( echo ERROR: build.gradle failed ^& pause ^& goto MENU )
...
git commit -m "chore: bump Android version to %NEW_VER% (versionCode %NEW_CODE%)"
...
if errorlevel 1 ( echo ERROR: push branch failed ^& pause ^& goto MENU )
...
echo   APK build: https://github.com/stevedaydream/local-food-collection/actions
echo   Release:   https://github.com/stevedaydream/local-food-collection/releases
echo ========================================
pause
goto MENU
...
echo   Actions: https://github.com/stevedaydream/local-food-collection/actions
echo ========================================
pause
goto MENU
